      ...
      <div className={`${isFormClicked ? 'flex flex-col font-semibold text-sm absolute top-[43px] z-40 w-full h-fit bg-white rounded-b-sm border-t border-slate-400 searchSuggestionShadow' :'hidden'} `}> 
          {
            storeList.filter((item) =>{
              ...
              const fullname = item._source.storeName.toLowerCase();
              return fullname.startsWith(searchTerm)
            }).map((item,ind)=>{
              return ind<12 && <div onClick={()=>navigate(`/shopProductPage/${item._source.shopId}`)} className='text-purple-900 hover:bg-blue-200/20 hover:text-buttonColor h-fit px-2 py-[6px]'>{item._source.storeName}</div>
            })
          }
      </div>
      </div>
